...
/**
 * Created by Shubhika on 11/24/2016.
 */
...
public class CosineCombinationIterator extends DisjunctionIterator implements ScoreIterator {
    ...
    public CosineCombinationIterator(NodeParameters parameters,
    ...
    public double score(ScoringContext c) {
        ...
        for (int i = 0; i < scoreIterators.length; i++) {
            ...
            String word = null;
            try {
                word = getAnnotatedNode(c).children.get(i).parameters;
            } catch (IOException e) {
                e.printStackTrace();
            }
            for(int j =0; j < scoreIterators.length; i++){
                if(j == i){
                    continue;
                }
                try {
                    if(word.equals(getAnnotatedNode(c).children.get(j).parameters)){
                        count = 1;
                    }
                } catch (IOException e) {
                    e.printStackTrace();
            ...
            denominator = score*score*score*score;
            densum += denominator;
            numsum += numerator;
            System.out.println("combine["+i+"]="+total);
        }
        total = numsum/Math.sqrt(densum);
        return total;
    }
